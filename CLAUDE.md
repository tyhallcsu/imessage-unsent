...
- **The pre-retract WAL page can be checkpointed away before the daemon snapshots it.** Long messages and slow unsends fail more often. Issue #67 / PR #68 add a rolling snapshot buffer at `~/Library/Application Support/imessage-unsent/wal-history/` that mitigates this. #169 fixed two defects that had made it inert: the buffer was copied into the archive after `recover.sh` ran, and the history scan was skipped whenever the live WAL was empty. The README has a "Limitations" section that's honest about what the tool cannot recover.

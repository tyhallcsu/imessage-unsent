...
- The protocol is a hard allowlist: `ping`, `status`, `recent` (read-only), plus `delete` and `compact`. **No socket op writes to the live `chat.db` or the daemon config.** `delete` and `compact` *do* mutate files, but only inside `~/Library/Application Support/imessage-unsent/archives/` (delete removes one recovery archive; compact drops the bulky `chat.db` snapshot family from an archive while keeping the recovered text + manifest). The archive `id` is validated against an anchored regex so it cannot traverse out of the archives directory. Any op outside the allowlist returns `{"ok":false,"error":{"code":"read_only",...}}`. Because the boundary is same-uid with no auth token, a process already running as your user could delete/compact archives directly on disk regardless of the socket — the socket does not widen that blast radius.
